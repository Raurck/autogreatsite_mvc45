...
namespace autogreatsite_mvc45.Upload
{
    public class FilesStatus
    {
        
        public const string HandlerPath = "/Upload/";
        const int tmbSize = 150;
        ...
        private string Scale(string imageFileName, int trgHeigth)
        {
            ...
            if (File.Exists(imageFileName))
            {
                ...
                using (Image img1 = new Bitmap(imageFileName))
                { 
                ...
                else if (((img1.Height - h) / (float)img1.Height) < -0.03)
                {
                    ...
                    using (var img3 = (img1 as Bitmap).Clone(cropRect, img1.PixelFormat))
                    {
                        using (Image img2 = img3.GetThumbnailImage(trgWidth, trgHeigth, callback, new IntPtr()))
                        {
